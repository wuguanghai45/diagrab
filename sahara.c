 ...
 *    Filename:  sahra.c
 *
 *    Description:  sahara protocol to catch SDX55 ram dump.
 *
 *
 *
 * =====================================================================================
...
#define dbg( log_level, fmt, arg... ) do {if (kickstart_options.verbose || LOG_ERROR == log_level) { unsigned msec = clock_gettime_msecs();  printf("[%03d.%03d] " fmt "\n",  msec/1000, msec%1000, ## arg);}} while (0)

static bool port_tx_data (void *buffer, size_t bytes_to_send) {
    size_t bytes_sent = safe_poll_write(com_port.port_fd, buffer, bytes_to_send, 1000);;
...
static bool sahara_rx_data(size_t bytes_to_read) {
    ...
    if (0 == bytes_to_read) {
        ...
        dbg(LOG_INFO, "Read %zd bytes, command %d and packet length %d bytes", temp_bytes_read, cpu_to_le32(command_packet_header->command), cpu_to_le32(command_packet_header->length));
        if (temp_bytes_read != sizeof(sahara_packet_header))
            return false;

        if (cpu_to_le32(command_packet_header->command) < SAHARA_LAST_CMD_ID) {
            dbg(LOG_EVENT, "RECEIVED <-- %s", boot_sahara_cmd_id_str[cpu_to_le32(command_packet_header->command)]);
            if (false == port_rx_data(sahara_data.rx_buffer + sizeof(sahara_packet_header), cpu_to_le32(command_packet_header->length) - sizeof(sahara_packet_header), &temp_bytes_read))
                return false;
            if (temp_bytes_read != (cpu_to_le32(command_packet_header->length) - sizeof(sahara_packet_header))) {
                dbg(LOG_INFO, "Read %zd bytes", temp_bytes_read + sizeof(sahara_packet_header));
                return false;
             }
        } else {
            dbg(LOG_EVENT, "RECEIVED <-- SAHARA_CMD_UNKONOW_%d", cpu_to_le32(command_packet_header->command));
            return false;
        }
    }
    else {
...
static bool send_reset_command ()
{
    ...
    sahara_reset->header.command = cpu_to_le32(SAHARA_RESET_ID);
    sahara_reset->header.length = cpu_to_le32(sizeof(sahara_packet_reset));
...
static bool send_memory_read_packet (uint64_t memory_table_address, uint64_t memory_table_length) {
    ...
    if (true == sahara_data.ram_dump_64bit) {
        sahara_memory_read_64bit->header.command = cpu_to_le32(SAHARA_64_BITS_MEMORY_READ_ID);
        sahara_memory_read_64bit->header.length = cpu_to_le32(sizeof(sahara_packet_memory_read_64bit));
        sahara_memory_read_64bit->memory_addr = cpu_to_le64(memory_table_address);
        sahara_memory_read_64bit->memory_length = cpu_to_le64(memory_table_length);
        ...
        sahara_memory_read->header.command	= cpu_to_le32(SAHARA_MEMORY_READ_ID);
        sahara_memory_read->header.length	= cpu_to_le32(sizeof(sahara_packet_memory_read));
        sahara_memory_read->memory_addr		= cpu_to_le32((uint32_t)memory_table_address);
        sahara_memory_read->memory_length	= cpu_to_le32((uint32_t)memory_table_length);
...
static bool sahara_start(void) {
    ...
    while (1)
    {
        ...
        case SAHARA_WAIT_HELLO:
          ...
          if (SAHARA_HELLO_ID != cpu_to_le32(sahara_hello->header.command)) {
                dbg(LOG_ERROR, "Received a different command: %x while waiting for hello packet", cpu_to_le32(sahara_hello->header.command));
            ...
            sahara_hello_resp->header.command = cpu_to_le32(SAHARA_HELLO_RESP_ID);
            sahara_hello_resp->header.length = cpu_to_le32(sizeof(sahara_packet_hello_resp));
            ...
            sahara_hello_resp->status = cpu_to_le32(SAHARA_STATUS_SUCCESS);
            sahara_hello_resp->mode = sahara_hello->mode;
            sahara_hello_resp->reserved0 = cpu_to_le32(1);
            sahara_hello_resp->reserved1 = cpu_to_le32(2);
            sahara_hello_resp->reserved2 = cpu_to_le32(3);
            sahara_hello_resp->reserved3 = cpu_to_le32(4);
            sahara_hello_resp->reserved4 = cpu_to_le32(5);
            sahara_hello_resp->reserved5 = cpu_to_le32(6);

              switch (cpu_to_le32(sahara_hello->mode)) {
                ...
                break;
                default:
                    dbg(LOG_EVENT, "RECEIVED <-- SAHARA_MODE_0x%x", cpu_to_le32(sahara_hello->mode));
                break;
              }

                if (cpu_to_le32(sahara_hello->mode) != sahara_data.mode) {
        ...
        case SAHARA_WAIT_COMMAND:
            ...
            if (SAHARA_MEMORY_DEBUG_ID == cpu_to_le32(((sahara_packet_header *)sahara_data.rx_buffer)->command)
				  || SAHARA_64_BITS_MEMORY_DEBUG_ID == cpu_to_le32(((sahara_packet_header *)sahara_data.rx_buffer)->command)) {
                dbg(LOG_EVENT, "RECEIVED <-- SAHARA_MEMORY_DEBUG");

				if (SAHARA_64_BITS_MEMORY_DEBUG_ID == cpu_to_le32(((sahara_packet_header *)sahara_data.rx_buffer)->command)) {
					sahara_data.ram_dump_64bit = true;
					dbg(LOG_EVENT, "Using 64 bit RAM dump mode");
					memory_table_addr = cpu_to_le64(sahara_memory_debug_64bit->memory_table_addr);
					memory_table_length = cpu_to_le64(sahara_memory_debug_64bit->memory_table_length);
				}
				else {
					sahara_data.ram_dump_64bit = false;
					memory_table_addr = cpu_to_le32(sahara_memory_debug->memory_table_addr);
					memory_table_length = cpu_to_le32(sahara_memory_debug->memory_table_length);
				}
                ...
                dbg(LOG_ERROR, "Received an unknown command: %d ", cpu_to_le32(((sahara_packet_header *)sahara_data.rx_buffer)->command));
                if (SAHARA_HELLO_ID == cpu_to_le32(((sahara_packet_header *)sahara_data.rx_buffer)->command))
                    continue;
                if (false == send_reset_command ()) {
                    return false;
                }
            ...
            if (memory_table_length > 0) {
                ...
                if (true == sahara_data.ram_dump_64bit) {
                    ...
                    for (i = 0; i < num_debug_entries; ++i) {
                        sahara_memory_table[i].save_pref = (uint64_t) cpu_to_le32(sahara_memory_table_rx[i].save_pref);
                        sahara_memory_table[i].mem_base = (uint64_t) cpu_to_le32(sahara_memory_table_rx[i].mem_base);
                        sahara_memory_table[i].length = (uint64_t) cpu_to_le32(sahara_memory_table_rx[i].length);
        ...
        case SAHARA_WAIT_RESET_RESP:
            ...
            if (true == sahara_rx_data(0)) {
                if (SAHARA_RESET_RESP_ID != cpu_to_le32(sahara_reset_resp->header.command)) {
                    dbg(LOG_INFO,"Waiting for reset response code %i, received %i instead.", SAHARA_RESET_RESP_ID, cpu_to_le32(sahara_reset_resp->header.command));
                    continue;
                }
            } else {
                if (SAHARA_RESET_RESP_ID == cpu_to_le32(sahara_reset_resp->header.command)) {
 ...
 int sahara_catch_dump(int port_fd, const char *path_to_save_files, int do_reset) {
    ...
    if (!strncmp(path_to_save_files, TFTP_F, strlen(TFTP_F))) {
        ...
        tftp_sock = tftp_write_request(kickstart_options.path_to_save_files+strlen(TFTP_F), "tftp_snd_blksize",
        ...
        if (tftp_sock > 0) {
            ...
            if (retval != (int)sahara_data.max_ram_dump_read-1) {
                close(tftp_sock);
                tftp_sock = -1;
            }
        }
